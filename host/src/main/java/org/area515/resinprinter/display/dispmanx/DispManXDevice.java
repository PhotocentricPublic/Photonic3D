	...
	private Memory showImage(Memory memory, BufferedImage image) {
		...
		try {
			...
			imageResourceHandle = DispManX.INSTANCE.vc_dispmanx_resource_create(
					VC_IMAGE_TYPE_T.VC_IMAGE_ARGB8888.getcIndex(), imageWidth.getValue(), imageHeight.getValue(),
			...
			int returnCode = DispManX.INSTANCE.vc_dispmanx_resource_write_data(imageResourceHandle,
					VC_IMAGE_TYPE_T.VC_IMAGE_ARGB8888.getcIndex(), imagePitch.getValue(), memory, sizeRect);
			if (returnCode != 0) {
				throw new IllegalArgumentException(
						"Couldn't vc_dispmanx_resource_write_data for dispmanx:" + returnCode);
			}
